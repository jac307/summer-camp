---
layout: default
title: Motion Capture and Animation Foundations
---

[Home](../../README.md)

---

## Part 1: 3D Art and Animation in Blender

Blender is a free and open-source 3D creation suite used by artists, designers, animators, and game developers. These beginner-friendly tutorials introduce the Blender interface, object manipulation, 3D modelling, materials, lighting, cameras, rigging, and animation.

The activities are designed to build foundational skills while encouraging creativity, experimentation, and play.

<a href="https://www.blender.org/download/" target="_blank" rel="noopener noreferrer"><strong>Download Blender</strong></a>

### 1. Introduction to Blender

* 🧱 [QuickStart Blender Guide](1_QuickStart_Blender_Guide.md)
* 🧱 [Blender Modifiers Reference Sheet](2_Blender_Modifiers.md)
* 🌀 [Object Play: Build an Abstract Sculpture or Creature](3_Object_Play_Activity.md)

### 2. Character Building

* 🧱 [Blender Reference: More Tools](4_Blender_Reference_More_Tools.md)
* ✏️ [Character Sketch Activity](5_Creature_Sketch_Activity.md)
* 👾 [Character Modelling: Session 1](6_Character_Modeling_Session_1.md)

### 3. Environment Building and Materials

* 🌆 [Environment Modelling Session](7_Environment_Modeling_Session.md)
* 💠 [Introduction to Materials](8_Intro_to_Materials.md)
* 👾 [Character Modelling: Session 2](9_Character_Modeling_Session_2.md)

### 4. Lighting and Cameras

* 💡 [Lighting and Camera](10_Lighting_Camera.md)

### 5. Rigging

* 🦴 [Introduction to Armatures and Rigging](11_Intro_to_Armatures.md)
* 🖍️ [Tips for Weight Painting in Blender](14_Weight_Painting.md)

### 6. Animation

* 🕹️ [Camera Animation Basics](12_Camera_Animation_Basics.md)
* 🕹️ [Mesh Animation Basics](13_Mesh_Animation_Basics.md)

### Additional Blender Resources

* 🔤 [Blender Shortcut Reference Guide](0_Shortcut_Reference_Guide.md)
* 👾 [Character Modelling: Additional References and Approaches](More_References_Character_Modeling.md)

---

## What Is 3D Art?

**3D art** uses three-dimensional digital space to create objects with height, width, and depth. Unlike a flat image, a 3D object can be viewed from different angles, lit with virtual lights, placed inside an environment, and animated over time.

In this program, participants use 3D art to design original objects, characters, and environments that can later be animated using keyframes or recorded motion-capture data.

---

## Instructor Presentation

<iframe
  src="W1_Teacher_Presentation_Slides.pdf"
  width="100%"
  height="600"
  title="Motion Capture and Animation Foundations presentation slides">
</iframe>

---

Credits: Jessica A. Rodríguez
